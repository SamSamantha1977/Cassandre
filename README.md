# Cassandra — mettre en commun la puissance disponible de nos ordinateurs

**Cassandra est un projet communautaire de calcul distribué.**

➡️ [Découvrir Cassandra et voir le compteur public](https://SamSamantha1977.github.io/Cassandre/)
...
Cassandra permet de mettre en commun cette capacité disponible afin de constituer collectivement une infrastructure de calcul pouvant servir à des projets trop lourds pour une seule machine.
...
> Cassandra n'utilise les ressources de l'ordinateur que lorsque son utilisateur ne s'en sert pas. Dès que l'utilisateur revient, sa machine redevient prioritaire.
...
La réponse habituelle consiste à construire ou louer toujours davantage d'infrastructures centralisées. Cassandra explore une autre voie : **utiliser intelligemment la capacité déjà présente dans les ordinateurs personnels de la communauté, uniquement lorsqu'ils ne sont pas utilisés par leur propriétaire.**
...
1. Vous installez le WORKER Cassandra sur votre ordinateur.
2. Vous continuez à utiliser votre ordinateur normalement.
3. Lorsque votre ordinateur est disponible et que vous ne l'utilisez pas, Cassandra peut lui confier de petites tâches adaptées à ses capacités.
4. Lorsque vous recommencez à utiliser votre ordinateur, Cassandra lui rend la priorité.

Le réseau peut ainsi exploiter temporairement des capacités CPU, GPU, RAM et de stockage réparties sur de nombreuses machines, sans demander aux participants de réserver leur ordinateur exclusivement à Cassandra.

## Vous gardez le contrôle de votre ordinateur

Le WORKER dispose d'une interface web locale destinée à son propriétaire.

Elle permet notamment de consulter l'état du WORKER, son planning, les paramètres laissés au choix de l'utilisateur et sa contribution au réseau Cassandra.

L'espace disque pouvant être mis à disposition reste configurable. Cassandra détermine automatiquement la capacité de calcul réellement disponible au moment où la machine peut participer.
...
Il comprend désormais des espaces **APPLICATIONS** et **CONNEXIONS IA**. L'architecture de connecteurs permet à des logiciels compatibles de soumettre des traitements à Cassandra via le WORKER lorsqu'un connecteur est explicitement activé. Un premier connecteur est prévu pour **Blender**, notamment pour les usages de création et de rendu 3D.
...
Chaque WORKER participant construit progressivement un **score de CONTRIBUTION** représentant sa participation cumulée au réseau Cassandra.
...
## Ce que Cassandra pourra rendre possible

Plus la toile Cassandra grandira, plus les projets qu'elle pourra soutenir deviendront ambitieux.
...
1. Ouvrez la [Release Cassandra Worker](https://github.com/SamSamantha1977/Cassandre/releases/tag/worker-bootstrap).
...
Cassandra est un projet communautaire récent et les versions actuellement publiées ne bénéficient pas encore d'une réputation de signature largement établie auprès de Windows.
...
Si l'avertissement apparaît pour le fichier téléchargé depuis la Release officielle Cassandra :
...
Cassandra est avant tout destiné à devenir une **communauté de personnes qui mettent en commun une partie de la capacité de leurs ordinateurs personnels lorsqu'elles ne les utilisent pas.**

Installer un WORKER, c'est ajouter une machine à cette capacité collective.

Aujourd'hui, Cassandra construit sa première communauté de participants. Chaque nouveau WORKER compte.

➡️ [Rejoindre Cassandra — télécharger le WORKER](https://github.com/SamSamantha1977/Cassandre/releases/tag/worker-bootstrap)
